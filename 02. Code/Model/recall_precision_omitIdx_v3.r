...
getBucket <- function(i){
    ...
    #TP <- unlist(lapply(X=1:length(cutoff), function(X){sum(score[1:50, i] >= cutoff[X])}))
    #FP <- unlist(lapply(X=1:length(cutoff), function(X){sum(score[-(1:50), i] >= cutoff[X])}))
    
    #recall <- TP/50
    #precision <- TP/(TP+FP)
    rePrec <- cbind(recall, precision)
    bucket <- cut(recall, breaks=seq(0, 1, 0.01), include.lowest=T,right=F, labels=1:100)
...
rec_prec_bucket <- function(data, n.bucket, infile, kernel){
    path <- paste("D:/RareDisease_TSC/Results/FV_2_update/2n_", kernel, "_", infile, "_huma_comb_addImpFeature/Rs/", kernel, sep='')
    ...
    write.xlsx(rec_prec_result, file=paste(kernel, '_', infile, "_", data, '_bucket', n.bucket, '_setRecPrec.xlsx',sep=''), row.names=F, append=T, showNA=T)
...
write.csv(table_result_1, 'D:\\jzhao\\RareDisease_TSC\\03. Output\\Aug10\\Summary_table_row_manifestComb.csv', row.names=F)
...
create_table4_forEachSim_baseSetting_v2 <- function(target_recall_list){
    score <- read.table(paste(inPath, '\\scoreTs.csv', sep=''), sep=',', header=T)[, -1]
    prec_target_list <- c(0.05, 0.1, 0.25, 0.5)
    score_pos <- unlist(lapply(1:ncol(score), function(i)score[1:50, i]))
    score_neg <- unlist(lapply(1:ncol(score), function(i)score[-(1:50), i]))
    score_pos_neg <- c(score_pos, score_neg)
    score <- score_pos_neg
    predobj <- prediction(score,c(rep(1, 500), rep(-1, length(score)-500)))
    perf <- performance(predobj, 'prec', 'rec')
    recall <- perf@x.values[[1]]
    precision <- perf@y.values[[1]]
    recPrec <- cbind(recall, precision)
    ...
    temp3 <- as.data.frame(t(as.data.frame(lapply(1:ncol(score), function(i){
        ...
        bucket <- cut(recall, breaks=seq(0, 1, 0.01), include.lowest=T,right=F)
        #rec_prec <- cbind(recall, precision)
        rec_prec_byBucket <- aggregate(recPrec, by=list(bucket), function(i)mean(i, na.rm=T))
        
        temp1 <- unlist(lapply(prec_target_list, function(X){
            idx <- which(abs(rePrec[, 2]-X)==min(abs(rec_prec_byBucket[, 3]-X), na.rm=T))
            rec_sel <- rec_prec_byBucket[idx, 2]
            return(rec_sel)
        }))        
...
table4_forEachSim_baseSetting_result <- create_table4_forEachSim_baseSetting_v2(c(0.5))
