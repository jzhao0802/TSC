...
# Date: 2015/06/09
# Developed by : jin Jin, Zhiyu Wang, Jie Zhao
...
subpath <- lapply(dir, function(X){
	...
	return(paste(root_path, X, sep='/'))
})
output_path_mod <- subpath[[2]]
output_path_rs <- subpath[[3]]
output_path_ind <- subpath[[1]]
...
Cost <- 'seq(0,10, 100)' 
crit <- ' greater than 0.25 modifiy'
######################################################
...
Max_Rec <- function(counter, Trail, cvec) {
  ...
  for (c in cvec ){
    ...
    # Create script files for validationunix_path
  ...
  ResMat <- c(counter, round(ROCAUC*100, 2), FP, TP, round(Recall*100, 2), round(100*PPV, 2))
...
getBucket <- function(n.buckt, score){
    ...
    bucket <- cut(recall, breaks=seq(0, 1, 1/n.bucket), include.lowest=T,right=F)
