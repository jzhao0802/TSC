...
log_path <- 'D:/Hui/TSC project/Max_recall/02_code'
input_path <- 'D:/RareDisease_TSC/Data/FV_2_update/2n'
svmperf_path <- 'D:/Hui/TSC project/Max_recall/02_code/svm_perf_0.75'
root_path <- 'D:/Hui/TSC project/Max_recall/03_moel'
...
Cost <- 'from 100 to 500 by 10 ' 
crit <- ' greater than 0.75 modifiy'
######################################################
...
Vl_ResMat <- read.csv(paste(out_path, '/Vl_ResMat_trail_',Trail,'.csv', sep=''), header=T)
# create prec-rec curve
PR_bucket <- function(data_type, Trail, out_path){
  
  index_path <- 'D:/RareDisease_TSC/Data/Dict_Ind_2_update/2n'
  ind_neg <- "/TestIndices"
  ind_pos <- "/TSCTestIndices"
  ind_vl_neg <-  '/VlIndices'
  ind_vl_pos <- '/TSCVlIndices'
  
  score_path <- paste('D:/Hui/TSC project/Max_recall/03_moel/Rs/result_trail_',Trail,sep='')
  ts_path <- '/ts_rs/ts_result_simulation_'
  vl_path <- '/vl_rs/vl_result_simulation_'
  ...
  if(data_type=='vl'){
    ...
    for(sim in 1:NumSimulation){
      
      #reading in postive
      pat_id_pos <- read.csv(paste(index_path, ind_vl_pos, sim, Ext, sep=''), header=F)
      
      #reading in negtive
      pat_id_neg <- read.csv(paste(index_path, ind_vl_neg, sim, Ext, sep=''), header=F)
      
      #reading in score
      score_all <- read.csv(paste(score_path, vl_path, sim, '/pred_C_', sel_c[sim], Ext, sep=''), header=F)
      
      pos_label <- rep(1, nrow(pat_id_pos))
      neg_label <- rep(-1, nrow(pat_id_neg))
      label <- c(pos_label, neg_label)
      pat_id_all <- rbind(pat_id_pos, pat_id_neg)
      scoreLabel1 <- cbind(label, score_all)
      ...
      temp_pr <- cbind(pat_id_all, recall, precision)
      
      pat_pr <- rbind(pat_pr, temp_pr)
  ...
  if(data_type=='ts'){
    ...
    for (sim in 1:NumSimulation){
      ...
      #reading in positive
      pat_id_pos <- read.csv(paste(index_path, ind_pos, sim, Ext, sep='' ), header=F)
      score_pos <-  read.csv(paste(score_path, ts_path, sim,'/pred_pos.csv', sep=''), header=F) 
      
      #reading in negative
      pat_id_neg <- numeric()
      score_neg <- numeric()
      for (sl in 1:NumSplit){
        temp_id <- read.csv(paste(index_path, ind_neg, sim, sl, Ext, sep=''), header=F)
        temp_score <- read.csv(paste(score_path, ts_path,sim,'/pred_neg', sl,Ext, sep=''), header=F)
        
        pat_id_neg <- rbind(pat_id_neg, temp_id)
        score_neg <- rbind(score_neg, temp_score)        
      ...
      neg_label <- rep(-1, nrow(pat_id_neg))
      label <- c(pos_label, neg_label)
      score_all <- rbind(score_pos, score_neg)
      pat_id_all <- rbind(pat_id_pos, pat_id_neg)
      scoreLabel1 <- cbind(label, score_all)
  ...
  final_pat_pr <- aggregate(pat_pr[,-1], list(pat_pr[,1]), 'mean')
  curve_order <- final_pat_pr[order(final_pat_pr$recall),] 
  
  bucket_num <- cut(curve_order$recall, 100, labels=seq(1, 100,1),include.lowest=T, right=F)
  curve_order_bucket <- cbind(bucket_num, curve_order)
  bucket_score <- aggregate(curve_order_bucket[,-1], list(curve_order_bucket[,1]), 'mean')[,-2]
  
  write.csv(bucket_score, paste(out_path, '/pre_rec_curve_100_',data_type,Ext, sep=''), quote=F, row.names=F)
  
}

tm <- Sys.time()
PR_bucket('vl', Trail, out_path )  
PR_bucket('ts', Trail, out_path )
