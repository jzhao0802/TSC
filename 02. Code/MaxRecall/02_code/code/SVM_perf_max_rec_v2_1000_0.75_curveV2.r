...
svmperf_path <- 'D:/jzhao/RareDisease_TSC/Max_recall/02_code/svm_perf_1000_0.25'
...
# pre-rec curve
Trail <- "2_0.65_maxRecall"

get_measure <- function(Trail, target){
    ...
    find_perfMeasure <-  function(n.bucket, target){
        recall_list <- lapply(1:10, function(i){
            ...
            recall_atTarPrec <-getBucket(n.bucket, score, nonTSCNum, target)
            return(recall_atTarPrec)
        })
...
m0 <- get_measure("1_0.25", target)
m1 <- get_measure("2_0.25_maxRecall", target)
m2 <- get_measure("2_0.65_maxRecall", target)
m3 <- get_measure("2_0.75_maxRecall", target)
load("D:/RareDisease_TSC/Results/FV_2_update/2n_lin_setting1_maxPPv_huma/Rs/lin/Ts/measure_std_2.RData", .GlobalEnv)
m_std <- recall_fnl
m <- rbind(m_std, m0, m1, m2, m3)
row.names(m) <- c('Standard SVM', 'Error Rate Loss', "Max recall Loss with 25% target precision", "Max recall Loss with 65% target precision", "Max recall Loss with 75% target precision")
colnames(m) <- c("Recall at 25% precision", "Recall at 65% precision", "Recall at 75% precision")
write.csv(m, "D:/jzhao/RareDisease_TSC/Max_recall/03_model/Rs/recall_comp_with_stdSVM_accum.csv", row.names=T)
