...
create_recallPrecision <- function(dataset){
    ...
    for(wt in wt_list){
        ...
        write.xlsx(rec_prec_result, paste(toupper(dataset),'_LR_rec_prec_byBucket_wt', wt, '.xlsx', sep=''), row.names=F)
...
par1_result <- create_recallPrecision('comb')
...
write.xlsx(par1, "part1_comb.xlsx")
...
rate_final <- create_rate('comb', 0.5)
rate_final <- create_rate('sep', 0.5)
...
par2 <- rate_final[, -1]
rownames(par2) <- NULL
write.xlsx(par2, "par2_sep.xlsx", row.names=F)
...
create_recallPrecision_addFlaggedPatient <- function(dataset, tar_recall){
    ...
    for(wt in wt_list){
        ...
        recPrec <- lapply(1:ncol(score), function(X){
            ...
            #rec_prec_byBucket_rbind <- rbind(rec_prec_byBucket_rbind, rec_prec_byBucket)
            ROCAUC <- performance(predobj, measure = "auc")@y.values[[1]]
            write.xlsx(rec_prec_byBucket, paste(toupper(dataset),'_LR_rec_prec_byBucket_wt', wt, '_sim', X, '.xlsx', sep=''),row.names=F)
            return(list(rec_prec_byBucket, ROCAUC))
        })
        auc_all <- unlist(lapply(1:length(recPrec), function(X){recPrec[[X]][[2]]}))
        auc_mean <- mean(auc_all)
