...
rec_prec_bucket <- function(data, n.bucket, infile, kernel, model){
    path <- paste("D:/RareDisease_TSC/Results/FV_2/2n_", kernel, "_", infile, "/Rs/", kernel, sep='')
    #path <- paste("D:/RareDisease_TSC/Results/FV_2_update/2n_", kernel, "_", infile, "_maxPPV_huma/Rs/", kernel, sep='')
    ...
    temp3 <- lapply(1:ncol(score), function(i){
        ...
        rec_prec <- cbind(recall, precision)
        rec_prec_byBucket <- aggregate(rec_prec, by=list(bucket), function(i)mean(i, na.rm=T))
        write.xlsx(rec_prec_result, paste('SVM_Linear_Base_Setting_ManifestComb_Curve_Sim', i, '.xlsx', sep=''), row.names=F)
        ...
        return(list(auc=auc, rec_prec_byBucket=rec_prec_byBucket, recall_list=temp4))
        
    })
    auc_mean <- mean(unlist(lapply(1:10, function(X)temp3[[X]][[1]])), na.rm=T)

    #recall_allSim_df<- lapply(1:10, function(X)return(rbind(aa,temp3[[X]][[3]])))
    #recall_allSim <- numeric()
    #for(i in 1:10){
       # aa <- temp3[[i]][[3]]
        #recall_allSim <- rbind(recall_allSim, aa)
    #}
    #library('plyr')
    ...
    #write.xlsx(rec_prec_result, paste('SVM_Linear_Base_Setting_ManifestComb_Curve', '.xlsx', sep=''), row.names=F)
    ...
    table_result_1 <- c(model, t(as.data.frame(table_result)))
    names(table_result_1) <-  c("Model", 'AUC', 'Recall at 5% precision', 'Recall at 10% precision', 'Recall at 25% precision', 'Recall at 50% precision', 'Recall for pos / neg (0.5 for LR) cut-off', 
    ...
    #write.csv(table_result_1, 'D:\\jzhao\\RareDisease_TSC\\03. Output\\Nov09\\Summary_table_row_', model,'.csv', row.names=F)
    return(table_result_1)
    
}

    

extra1 <- rec_prec_bucket('Ts', 100,  infile='model1', kernel='lin', "Base_Setting")
extra3 <- rec_prec_bucket('Ts', 100,  infile='model3', kernel='lin', "Bigger_C")
extra6 <- rec_prec_bucket('Ts', 100,  infile='model6', kernel='lin', "Smaller_C")
extra7 <- rec_prec_bucket('Ts', 100,  infile='model7', kernel='lin', "Bigger_C_Wider_wt")
extra9 <- rec_prec_bucket('Ts', 100,  infile='model9_v2', kernel='lin', "Unlimit_FP")
extra21 <- rec_prec_bucket('Ts', 100,  infile='model1', kernel='rbf', "Base_Setting")
extra23 <- rec_prec_bucket('Ts', 100,  infile='model3', kernel='rbf', "Bigger_C")
extra_all <- rbind(extra1, extra3, extra6, extra7, extra9, extra21, extra23)
write.xlsx(extra_all, 'D:\\jzhao\\RareDisease_TSC\\03. Output\\Nov09\\refresh_forSVM.xlsx', row.names=F)
