...
##################################################################################################
  ...
  TrTSCPtId <- PtIdTSC[TrTSCIndex]
  VlTSCPtId <- PtIdTSC[VlTSCIndex]
  TsTSCPtId <- PtIdTSC[TsTSCIndex]
  
  TrVlTSCPtId <- c(TrTSCPtId,VlTSCPtId)
...
##################################################################################################
  ...
  NumPtNonTSC <- length(PtIdNonTSC)
  
  NumTrPtNonTSC <- round(PERC * NumPtNonTSC)
    
  NumOtPtNonTSC <- NumPtNonTSC - NumTrPtNonTSC
  
  NumVlPtNonTSC <- constNumVlPtNonTSC
  
  NumTsPtNonTSC <- NumOtPtNonTSC - NumVlPtNonTSC
  ...
  if (NumTrPtNonTSC > 0){
    TrNonTSCPtId <- PtIdNonTSC[TrNonTSCIndex]
    write.table(TrNonTSCPtId,file=TrIndexFile,quote=FALSE,col.names=FALSE,row.names=FALSE,append=TRUE)    
  }

  VlNonTSCPtId <- PtIdNonTSC[VlNonTSCIndex]
  
  TsNonTSCPtId <- PtIdNonTSC[TsNonTSCIndex]  
...
################################################################################################## 
  ...
  if (PARALLEL) {
    ...
    sfInit(parallel=TRUE, cpus=num_pros, type="SOCK",slaveOutfile = "D:/RareDisease_TSC/temp/error.log")
    sfLibrary(snowfall)
  }
