...
##################################################################################################   
  ...
  ifelse(ONE,prmv <- nuv,prmv<- seev)
  ifelse(ONE,svmtype <- "one-classification",svmtype <- "C-classification")
  #ifelse(ONE,wtslst <- list(NULL),wtslst <- list(c("-1"=1,"1"=1),  c("-1"=1,"1"=1) ))
  #ifelse(ONE,wtslst <- list(NULL),wtslst <- list(c("-1"=.05,"1"=1), c("-1"=.1,"1"=1) ))
   ...
   # ifelse(ONE,wtslst <- list(NULL),wtslst <- list(c("-1"=.01,"1"=1), c("-1"=.02,"1"=1), c("-1"=.03, "1"=1), c("-1"=.04, "1"=1), c("-1"=.05, "1"=1), c("-1"=.06,"1"=1), c("-1"=.07,"1"=1), c("-1"=.08, "1"=1), c("-1"=.09, "1"=1),    
    ...
    for (iprm in 1 : length(prmv) ) {
      ...
      #for (iwt in 1: wtctr) {  
        ...
        svm_model <- svm(x = trainv$x, y = trainv$y, scale = FALSE, type = svmtype, nu = prmv[iprm],  cost= prmv[iprm],  kernel = kertype )
...
#add plot end
    ...
    if (!invalid(validResLst))
      selwt <- validResLst[[selIndex]]
   	valid_auc <- c(counter, selROCAUC, selDR, selTP, selPPV, selprm) #added by Jie
	
		CselectedFile <-  "D:/RareDisease_TSC/Results/FV_2/2n_lin_noweight/Rs/lin/CselectedFile.csv" #added by Jie
    ...
    svm_model <- svm(x = train$x, y = train$y, scale = FALSE, type = svmtype, nu = selprm,  cost = selprm, kernel = kertype)
	    ...
	    get_perform <- function(data){
			svm_model <- svm(x = data$x, y = data$y, scale = FALSE, type = svmtype, nu = selprm,  cost = selprm, kernel = kertype)
			...
			#SV <- svm_model$SV
			 if (!ONE) {
				...
				auc <- c(counter, ROCAUC, DR, TP, PPV, selprm) #added by Jie
	...
	for (iprm in 1 : length(prmv) ) {
		#for (iwt in 1: wtctr) {  
					...
					if (!ONE) {
						...
						#cat(" Training ROCAUC: ", ROCAUC, " Training DR: ", DR, " Training TP: ", TP, "\n")  
						#cat(file=TrRsFile,append=TRUE, "Training ROCAUC: ", ROCAUC, " Training DR: ", DR, " Training TP: ", TP, "\n")
						trainv_auc_i <- c(counter, ROCAUC, DR, TP, PPV, prmv[iprm]) #added by Jie
					...
					if (ONE) {
						...
						#cat("Training TP ",TP, "\n")
						#cat(file=TrRsFile,append=TRUE,"Training TP ", TP, "\n")
						trainv_auc_sim <- c(counter, ROCAUC, DR, TP, PPV, selprm) #added by Jie
    ...
    if (!invalid(validResLst))
      selwt <- validResLst[[selIndex]]
   	valid_auc <- c(counter, selROCAUC, selDR, selTP, selPPV, selprm, selwt[1], selgamma) #added by Jie
    		...
    		CselectedFile <-  "D:/RareDisease_TSC/Results/FV_2/2n_rbf/Rs/rbf/CselectedFile.csv"
