...
##################################################################################################   
  ...
  if (kertype == "linear") {
  seev <- c(50,  100, 150) # a set of values for C for binary SVMs
  #seev <- seq(50, 300, 10)
  ifelse(ONE,prmv <- nuv,prmv<- seev)
  ifelse(ONE,svmtype <- "one-classification",svmtype <- "C-classification")
  #ifelse(ONE,wtslst <- list(NULL),wtslst <- list(c("-1"=.05,"1"=1),  c("-1"=.1,"1"=1) ))
  #ifelse(ONE,wtslst <- list(NULL),wtslst <- list(c("-1"=.05,"1"=1), c("-1"=.1,"1"=1) ))
  ifelse(ONE,wtslst <- list(NULL),wtslst <- list(c("-1"=.05,"1"=1), c("-1"=.1,"1"=1), c("-1"=.06, "1"=1), c("-1"=.07, "1"=1), c("-1"=.08, "1"=1), c("-1"=.09, "1"=1),c("-1"=.1, "1"=1)  ))
    ...
    if (!invalid(validResLst))
      selwt <- validResLst[[selIndex]]
   	valid_auc <- c(counter, selROCAUC, selDR, selTP, selPPV, selprm, selwt[1]) #added by Jie
		...
		CselectedFile <-  "D:/RareDisease_TSC/Results/FV_2/2n_lin_model7/Rs/lin/CselectedFile.csv" #added by Jie
