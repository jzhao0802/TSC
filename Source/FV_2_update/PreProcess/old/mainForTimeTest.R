...
##################################################################################################
  ...
  cat(file=paste(TimeLog, '/timeLogComp', Ext, sep=''),append=TRUE,'main:twoClass:time used for sfExport: ', timeUsed, '\n')

  
simu1 <- 1
simu2 <- 10
cat(file=paste(TimeLog, '/timeLogComp', Ext, sep=''),append=TRUE, 'main.R:twoClass:Simulation(not parallel) start:', format(Sys.time(), "%a %b %d %X %Y"), '\n')

timeStart <- proc.time()
  for (counter in simu1:simu2) {
    tm <- proc.time()
...
cat(file=paste(TimeLog, '/timeLogComp', Ext, sep=''),append=TRUE,'main:twoClass: the whole process end:', format(Sys.time(), "%a %b %d %X %Y"), '\n')
timeUsed <- proc.time()-timeStart
cat(file=paste(TimeLog, '/timeLogComp', Ext, sep=''),append=TRUE,'main:twoClass:simulation', simu1, ':', simu2,'time used for simulation parallel: ', timeUsed, '\n')
