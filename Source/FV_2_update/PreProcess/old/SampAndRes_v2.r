...
################################################################################################## 
  ...
  PARALLEL <- TRUE   # Set PARALLEL to FALSE for swquential processing 
if (PARALLEL) {
    num_pros <- Sys.getenv('NUMBER_OF_PROCESSORS')
    sfInit(parallel=TRUE, cpus=num_pros, type="SOCK",slaveOutfile = "D:/jzhao/RareDisease_TSC/01. Data/FV_2/2n/error.log")
    sfLibrary(snowfall)
  }
 ...
 # Split non-TSC data and perfrom parallel processing
  ...
  if(loop==1){
		cat(file=paste(TimeLog, '/timeLogComp', Ext, sep=''),append=TRUE,'main_v2:twoClass:sim1:Testneg sfExport start:', format(Sys.time(), "%a %b %d %X %Y"), '\n')
  }
sfExport("IdxAry","Dict","NonTSCAgeGender","NonTSCCodeDrug","MeanAgeFirst","MeanAgeSecond","TsFilePos","TsFileNeg","filenum","TsIndexFile","Ext","norm2")
  sfExport("NormAge","MkDictEnv","MkFtVec","PrsTst")
  sfExport("ldply", namespace = "plyr")
  sfExport("quickdf", namespace = "plyr")
if(loop==1){
	cat(file=paste(TimeLog, '/timeLogComp', Ext, sep=''),append=TRUE,'main_v2:twoClass:sim1:Testneg parallel start:', format(Sys.time(), "%a %b %d %X %Y"), '\n')	
}
  
  sfClusterApplyLB(1:NumSplit,PrsTst) 
if(loop==1){
	cat(file=paste(TimeLog, '/timeLogComp', Ext, sep=''),append=TRUE,'main_v2:twoClass:sim1:Testneg parallel end:', format(Sys.time(), "%a %b %d %X %Y"), '\n')	
}  
