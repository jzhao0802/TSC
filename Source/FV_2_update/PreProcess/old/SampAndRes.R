...
if (PARALLEL) {
 ...
 # Split non-TSC data and perfrom parallel processing
  ...
  if(loop==1){
		cat(file=paste(TimeLog, '/timeLogComp', Ext, sep=''),append=TRUE,'main:twoClass:sim1:Testneg sfExport start:', format(Sys.time(), "%a %b %d %X %Y"), '\n')
  }
...
if(loop==1){
	cat(file=paste(TimeLog, '/timeLogComp', Ext, sep=''),append=TRUE,'main:twoClass:sim1:Testneg parallel start:', format(Sys.time(), "%a %b %d %X %Y"), '\n')	
}
  
  sfClusterApplyLB(1:NumSplit,PrsTst) 
if(loop==1){
	cat(file=paste(TimeLog, '/timeLogComp', Ext, sep=''),append=TRUE,'main:twoClass:sim1:Testneg parallel end:', format(Sys.time(), "%a %b %d %X %Y"), '\n')	
}  
