...
if(!file.exists(pathOutput)){
 ...
 # Source files
  ...
  sfSource("D:/RareDisease_TSC/Source/FV_2_update/Model/InvkSVM_VarImp.R")
...
RsFileTr <- "D:/RareDisease_TSC/Results/FV_2_update/2n_lin/Rs/lin/ResTrFile"
RsFileTrAllSet <- "D:/RareDisease_TSC/Results/FV_2_update/2n_lin/Rs/lin/ResTrSetAllFile"
RsFileTrVl <- "D:/RareDisease_TSC/Results/FV_2_update/2n_lin/Rs/lin/ResTrVlFile"
CoefFileTrVl <- "D:/RareDisease_TSC/Results/FV_2_update/2n_lin/Rs/lin/CoefTrVlFile"
CoefsFileTrVl <- "D:/RareDisease_TSC/Results/FV_2_update/2n_lin/Rs/lin/CoefsTrVlFile"
SVFileTrVl <- "D:/RareDisease_TSC/Results/FV_2_update/2n_lin/Rs/lin/SVTrVlFile"
...
parOnSim <- function(counter) {
  TrFile1 <- "D:/RareDisease_TSC/Data/FV_2/2n_comb/trainv"
  TrFile <- "D:/RareDisease_TSC/Data/FV_2/2n_comb/train"
  VlFile <- "D:/RareDisease_TSC/Data/FV_2/2n_comb/valid"
  TsFilePos <- "D:/RareDisease_TSC/Data/FV_2/2n_comb/testpos"
  TsFileNeg <- "D:/RareDisease_TSC/Data/FV_2/2n_comb/testneg"
  TrainDict <- 'D:\\RareDisease_TSC\\Data\\Dict_Ind_2\\2n_comb\\CodesDrugsDict'
  ...
  TrainDict <- paste(TrainDict, counter, Ext, sep="")
  
  dict <- read.table(TrainDict, sep=' ', header=T)
  ...
  resultInvkSVM <- InvkSVM(TrFile1,VlFile,TrFile,kertype,ONE,TrVlTsRsFile,ThreshLim,counter,ModelFile, limitDr, dict)
  ...
  ResMat[counter,2] <- DR
  ResMat[counter,3] <- TP
  result <- list(ResMat=ResMat, NumDiag=length(TsPosTarget), NumUnDiag=NumUnDiag, scoreVl=scoreVl, predscoreTs=predscore, aucVl=aucVl, aucTr=aucTr, aucTrAllSet=aucTrAllSet, aucTrVl=aucTrVl, coefTrVl=coefTrVl)  #added by Jie to store the score of test data
  return(result)
  #print(proc.time()- tm)
...
#get the coefficient of feature for all simulations
coef1 <- lapply(1:10, function(X){length(result[[X]][[10]]@ra)})
library(dplyr)
coef <- ldply(coef1, quickdf)
...
lapply(1:10, function(i)dim(result[[i]][[10]]))

save(coefTrVl, file=paste(TimeLog, '/Coefficient_10Simulation_list.RData', sep=''))
...
#get top30 feature list

# method 1: average by 
library(xlsx)

outpath <- 'D:\\RareDisease_TSC\\Results\\FV_2_update\\2n_lin\\Rs\\lin'
setwd(outpath)

get_topFeature <- function(pre_top, top, method){
    if(method=='Avg'){
        coef_avg1 <- aggregate(as.numeric(coefTrVl[, 3]), by=list(coefTrVl[, 2]), mean)
        feature_cnt_top_30 <- coef_avg1[order(abs(coef_avg1[, 2]), decreasing=T)[1:top], ]
        
        
    }else if(method=='Avg_abs'){
        coef_avg2 <- aggregate(abs(as.numeric(coefTrVl[, 3])), by=list(coefTrVl[, 2]), mean)
        feature_cnt_top_30 <- coef_avg2[order(abs(coef_avg2[, 2]), decreasing=T)[1:top], ]
        
    }else if(method=='Cnt'){
        # method 2: number of falling in top50
        coef_top50_list <- lapply(1:10, function(X){
            temp <- result[[X]][[10]][1:pre_top,]
            return(temp)
        })
        
        top50_allSim <- numeric()
        for (i in 1:10){
            top50_allSim <- rbind(top50_allSim, coef_top50_list[[i]])
        }
        
        feature_cnt <- aggregate(rep(1, nrow(top50_allSim)), by=list(top50_allSim[, 1]), sum)
        feature_cnt_top_30 <- feature_cnt[order(feature_cnt[, 2], decreasing=T)[1:top], ]
    }else if(method=='Cnt_variation'){
        rank_list <- lapply(1:10, function(X){
            temp <- result[[X]][[10]]
            feature <-  temp[order(abs(as.numeric(temp[, 2])), decreasing=T), 1]
            rank <- 1:nrow(temp)
            feature_rank <- cbind(feature, rank)
            return(feature_rank)
        })
        feature_rank_10sim <- numeric()
        for(i in 1:10){
            feature_rank_10sim <- rbind(feature_rank_10sim, rank_list[[i]])
        }
        rank_avg <- aggregate(as.numeric(feature_rank_10sim[, 2]), by=list(feature_rank_10sim[, 1]), mean)
        rank_avg_top30 <- rank_avg[order(rank_avg[, 2]),][1:100,]
        feature_cnt_top_30 <- rank_avg_top30
    }else if(method=='Cnt_variation_noAbs'){
        rank_list <- lapply(1:10, function(X){
            temp <- result[[X]][[10]]
            feature <-  temp[order(as.numeric(temp[, 2]), decreasing=T), 1]
            rank <- 1:nrow(temp)
            feature_rank <- cbind(feature, rank)
            return(feature_rank)
        })
        ...
        rank_avg <- aggregate(as.numeric(feature_rank_10sim[, 2]), by=list(feature_rank_10sim[, 1]), mean)
        rank_avg_top30 <- rank_avg[order(rank_avg[, 2]),]
        feature_cnt_top_30 <- rank_avg_top30
        
    }else if(method=="Cnt_variation_noAbs_forExtraLRCFeature"){
        rank_list <- lapply(1:10, function(X){
            temp <- result[[X]][[10]]
            feature <-  temp[order(as.numeric(temp[, 2]), decreasing=T), 1]
            rank <- 1:nrow(temp)
        ...
        extra<-c(paste("C",1:11,sep=""),manifest_age_combo)
        
        extra_rank <- feature5[feature5[, 1] %in% extra,]
        feature_cnt_top_30 <- extra_rank
    }else{
        stop("please check your 'method' parameter!\n")
    }    
    
    
    write.xlsx(feature_cnt_top_30 , 'Top_feature_table_update.xlsx', sheetName=paste('method_', method, sep=''), row.names=F, append=T, showNA=T)
    return(feature_cnt_top_30)
    
}
feature1 <- get_topFeature(50, 31, 'Avg')
feature2 <- get_topFeature(50, 31, 'Avg_abs')
feature3 <- get_topFeature(50, 30, 'Cnt')
feature4 <- get_topFeature(50, 30, 'Cnt_variation')
feature4 <- get_topFeature(50, 30, 'Cnt_variation_noAbs_forExtraLRCFeature')
feature5 <- get_topFeature(50, 30, 'Cnt_variation_noAbs')
#for extra LR features in SVM_comb (21)
manifest_age_combo<-sort(paste(paste("C",7:11,sep=""),rep(1:2,each=5),sep="_"))[c(5:10,1:4)]
extra<-c(paste("C",1:11,sep=""),manifest_age_combo)

extra_rank <- feature5[feature5[, 1] %in% extra,]

write.xlsx(extra_rank, "")
