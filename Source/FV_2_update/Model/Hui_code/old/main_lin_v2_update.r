...
##################################################################################################
 ...
 # Source files
  ...
  sfSource("D:/RareDisease_TSC/Source/FV_2_update/Model/Hui's code/InvkSVM.R")
  sfSource("D:/RareDisease_TSC/Source/FV_2_update/Model/Hui's code/PredSVM.R")
...
AvgRsFile <- "D:/RareDisease_TSC/Results/FV_2_update/2n/Rs/lin/AvgResFile"
Ext <- '.csv'
AvgRsFile <- paste(AvgRsFile,Ext,sep="")
timeStart<- proc.time()

num_pros <- Sys.getenv('NUMBER_OF_PROCESSORS')
sfInit(parallel=TRUE, cpus=num_pros, type="SOCK",slaveOutfile = "D:/RareDisease_TSC/Results/FV_2_update/logModel_binary_lin_parOnSimAll.txt")
sfLibrary(snowfall)
...
TimeLog <- "D:/RareDisease_TSC/Results/FV_2_update/2n/Rs/lin"
...
parOnSim <- function(counter) {
  ...
  TSCIndexFile  <- "D:/RareDisease_TSC/Results/FV_2_update/2n/Indices/lin/ProspectiveTSC"
  ConTSCIndexFile  <- "D:/RareDisease_TSC/Results/FV_2_update/2n/Indices/lin/ConfirmedTSC"
  
  ModelFile <- "D:/RareDisease_TSC/Results/FV_2_update/2n/Model/lin/SVModel"
  
  TrVlTsRsFile <- "D:/RareDisease_TSC/Results/FV_2_update/2n/Rs/lin/TrVlTsResFile"
  ...
  if (PARALLEL) {
    num_pros <- Sys.getenv('NUMBER_OF_PROCESSORS')
    sfInit(parallel=TRUE, cpus=num_pros, type="SOCK",slaveOutfile = "D:/RareDisease_TSC/Results/FV_2_update/logModel_binary_lin_parOnSim.txt")
    sfLibrary(snowfall)
  }
  ...
  ResMat[counter,2] <- DR
  ResMat[counter,3] <- TP
  result <- list(ResMat=ResMat, NumDiag=length(TsPosTarget), NumUnDiag=NumUnDiag)
  return(result)
  #print(proc.time()- tm)
...
NumDiag <- result[[1]][[2]]
  ...
  write.table(ResMat1,file = AvgRsFile,sep=',')
