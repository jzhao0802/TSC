...
InvkSVM <- function(TrFile1,VlFile,TrFile,kertype,ONE,TrRsFile,ThreshLim,counter,ModelFile) {  
  ...
  ifelse(ONE,wtslst <- list(NULL),wtslst <- list( c("-1"=.0001,"1"=1), c("-1"=.01,"1"=1),c("-1"=.05,"1"=1), c("-1"=.1,"1"=1) ) )
...
##################################################################################################   
  ...
  if (kertype == "linear") {
    ...
    selprm <- validResMat[selIndex,1]
		DNumFile <-  "D:/RareDisease_TSC/Results/FV_2_update/2n/Rs/lin/DNumRecord.csv"
    ...
    if (!invalid(validResLst))
      selwt <- validResLst[[selIndex]]
		...
		CselectedFile <-  "D:/RareDisease_TSC/Results/FV_2_update/2n/Rs/lin/CselectedFile.csv"
...
################################################################################################## 
  ...
  if (kertype == "radial") {
    ...
    selgamma <- validResMat[selIndex,2]
			DNumFile <-  "D:/RareDisease_TSC/Results/FV_2_update/2n/Rs/rbf/DNumRecord.csv"
   		cat(file=DNumFile, append=TRUE, 'Simulation ', counter, ': Validation set selDR = ', selDR,'\n')
