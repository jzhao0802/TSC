...
setting <- 'setting4'
version <- 'huma'
pathOutput <- paste("D:/RareDisease_TSC/Results/FV_2_update/2n_rbf_", setting, '_', version, sep='')
dir <- c('Indices', 'Model', 'Rs')

#unlink(pathOutput, recursive=T)
lapply(1:3, function(X)dir.create(path=paste(pathOutput, dir[X], 'rbf', sep='/'), recursive=T, showWarnings=T))
...
AvgRsFile <- paste(pathOutput, "/Rs/rbf/AvgResFile", sep='')
RsFileVl <- paste(pathOutput, "/Rs/rbf/ResVlFile", sep='')
Ext <- '.csv'
...
sfInit(parallel=TRUE, cpus=num_pros, type="SOCK",slaveOutfile = paste(pathOutput, "/logModel_binary_rbf_parOnSimAll.txt", sep=''))
sfLibrary(snowfall)
sfExport('ResMat', 'PARALLEL', 'ONE','ThreshLim', 'kertype', 'InvkSVM','PredSVM', 'Ext', 'pathOutput')
    sfClusterEval(library("e1071"))
	sfClusterEval(library("ROCR"))
	sfClusterEval(library("pracma"))
	sfClusterEval(library("gtools"))
	sfClusterEval(library("MASS"))
TimeLog <-paste(pathOutput, "/Rs/rbf", sep='')
...
parOnSim <- function(counter) {
  ...
  TSCIndexFile  <- paste(pathOutput, "/Indices/rbf/ProspectiveTSC", sep='')
  ConTSCIndexFile  <- paste(pathOutput, "/Indices/rbf/ConfirmedTSC", sep='')
  
  ModelFile <- paste(pathOutput, "/Model/rbf/SVModel", sep='')
  
  TrVlTsRsFile <-paste(pathOutput, "/Rs/rbf/TrVlTsResFile", sep='')
  ...
  if (PARALLEL) {
    num_pros <- Sys.getenv('NUMBER_OF_PROCESSORS')
    sfInit(parallel=TRUE, cpus=num_pros, type="SOCK",slaveOutfile = paste(pathOutput, "/logModel_binary_rbfKernel_parOnSim.txt", sep=''))
    sfLibrary(snowfall)
  }
...
predScore <- matrix(nr=NumUnDiag+50, nc=10)
...
plotf <- function(data){
	pdf(file=paste(pathOutput, '/recall-precision-curve_simulation_', data, '.pdf', sep=''))
  par(mfrow=c(5,2))
  par(cex.main=1.3, cex.lab=1.2, cex.axis=1)
