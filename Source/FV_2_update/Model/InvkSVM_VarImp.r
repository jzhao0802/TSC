...
InvkSVM <- function(TrFile1,VlFile,TrFile,kertype,ONE,TrRsFile,ThreshLim,counter,ModelFile, limDr, dict) {  
...
##################################################################################################   
  ...
  ifelse(ONE,svmtype <- "one-classification",svmtype <- "C-classification")
  ifelse(ONE,wtslst <- list(NULL),wtslst <- list(c("-1"=.05,"1"=1),  c("-1"=.1,"1"=1) ))
  #ifelse(ONE,wtslst <- list(NULL),wtslst <- list(c("-1"=.1,"1"=1)))
  #ifelse(ONE,wtslst <- list(NULL),wtslst <- list(c("-1"=.05,"1"=1), c("-1"=.1,"1"=1) ))
...
#add plot end
	...
	get_perform <- function(data){
            ...
            coef_featureIdx <- coef@ja
            coef_value <- coef@ra
            #coef_value_top100 <- coef_value[order(abs(coef_value), decreasing=T)[1:100]]
            
			manifest_age_combo<-sort(paste(paste("C",7:11,sep=""),rep(1:2,each=5),sep="_"))[c(5:10,1:4)]
			demo <-c("Id","FirstAge", "LastAge", "Male","Female",paste("C",1:11,sep=""),manifest_age_combo)
			#dict_c1 <- 1:(nrow(dict)+6)
			dict_c2 <- c(demo, as.vector(dict[, 2]), '999')
			
            feature_list_order <- dict_c2[coef_featureIdx[order(abs(coef_value), decreasing=T)]]
            coef_value_order <- coef_value[order(abs(coef_value), decreasing=T)]
            feature_coef_order <- cbind(Feature=feature_list_order, Coef=coef_value_order)
...
trainv_auc_allSet <- get_perform_trainv_all()
	...
	result_invkSVM <- list()
	result_invkSVM <- list(model=svm_model, score=predscoreVl, valid_auc=valid_auc,  trainv_auc=trainv_auc, trainv_auc_allSet=trainv_auc_allSet, trainValid_auc=trainValid_auc, trainValid_coef=trainValid_coef)
...
#add plot end
    ...
    selgamma <- validResMat[selIndex,2]
		#valid_auc <- c(counter, selROCAUC, selDR, selTP, selPPV) #added by Jie
			...
			DNumFile <- paste(pathOutput, '/Rs/rbf/DNumRecord.csv', sep='') 
   		cat(file=DNumFile, append=TRUE, 'Simulation ', counter, ': Validation set selDR = ', selDR,'\n')
    ...
    if (!invalid(validResLst))
      selwt <- validResLst[[selIndex]]
   	valid_auc <- c(counter, selROCAUC, selDR, selTP, selPPV, selprm, selwt[1], selgamma) #added by Jie
    		...
    		CselectedFile <-  paste(pathOutput, '/Rs/rbf/CselectedFile.csv', sep='')
