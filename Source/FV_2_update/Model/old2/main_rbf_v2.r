...
AvgRsFile <- "D:/RareDisease_TSC/Results/FV_2_update/2n/Rs/rbf/AvgResFile"
Ext <- '.csv'
AvgRsFile <- paste(AvgRsFile,Ext,sep="")  
timeStart <- proc.time()
num_pros <- Sys.getenv('NUMBER_OF_PROCESSORS')
sfInit(parallel=TRUE, cpus=num_pros, type="SOCK",slaveOutfile = "D:/RareDisease_TSC/Results/FV_2_update/logModel_binary_rbf_parOnSimAll.txt")
sfLibrary(snowfall)
...
TimeLog <- "D:/RareDisease_TSC/Results/FV_2_update/2n/Rs/rbf"
cat(file=paste(TimeLog, '/timeLogComp', Ext, sep=''),append=TRUE,'main_rbf_v2:oneClass=', ONE, 'time used for sfExport1: ', proc.time()-timeStart, 'and the time is ', format(Sys.time(), "%a %b %d %X %Y"), '\n')
...
parOnSim <- function(counter) {
  ...
  TSCIndexFile  <- "D:/RareDisease_TSC/Results/FV_2_update/2n/Indices/rbf/ProspectiveTSC"
  ConTSCIndexFile  <- "D:/RareDisease_TSC/Results/FV_2_update/2n/Indices/rbf/ConfirmedTSC"
  
  ModelFile <- "D:/RareDisease_TSC/Results/FV_2_update/2n/Model/rbf/SVModel"
  
  TrVlTsRsFile <- "D:/RareDisease_TSC/Results/FV_2_update/2n/Rs/rbf/TrVlTsResFile"
  ...
  if (PARALLEL) {
    num_pros <- Sys.getenv('NUMBER_OF_PROCESSORS')
    sfInit(parallel=TRUE, cpus=num_pros, type="SOCK",slaveOutfile = "D:/RareDisease_TSC/Results/FV_2_update/logModel_binary_rbf_parOnSim.txt")
    sfLibrary(snowfall)
  }
  ...
  sfClusterEval(library("e1071"))
  cat(file=paste(TimeLog, '/timeLogComp', Ext, sep=''),append=TRUE,'main_rbf_v2:oneClass=', ONE, 'time used for sfExport1+2: ', proc.time()-timeStart, 'and the time is ', format(Sys.time(), "%a %b %d %X %Y"), '\n')

  # Number of files which contain negative test data
  NumSplit <- 8 
  
  timeStartSim <- proc.time()
  upredscore <- sfClusterApplyLB(1:NumSplit,PredSVM)
  cat(file=paste(TimeLog, '/timeLogComp', Ext, sep=''),append=TRUE,'main_rbf_v2:oneClass=', ONE, 'time used for parallel on 10 simulation: ', proc.time()-timeStartSim, 'and the time is ', format(Sys.time(), "%a %b %d %X %Y"), '\n')
