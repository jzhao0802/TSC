...
InvkSVM <- function(TrFile1,VlFile,TrFile,kertype,ONE,TrRsFile,ThreshLim,counter,ModelFile) {  
  ...
  ifelse(ONE,wtslst <- list(NULL),wtslst <- list(c("-1"=.05,"1"=1)) )
...
#add plot end
    ...
    selprm <- validResMat[selIndex,1]
		DNumFile <-  "D:/RareDisease_TSC/Results/FV_2_update/2n_lin/Rs/lin/DNumRecord.csv"
    ...
    if (!invalid(validResLst))
      selwt <- validResLst[[selIndex]]
		...
		CselectedFile <-  "D:/RareDisease_TSC/Results/FV_2_update/2n_lin/Rs/lin/CselectedFile.csv"
	...
	result_invkSVM <- list()
	result_invkSVM <- list(model=svm_model, score=predscoreVl)
