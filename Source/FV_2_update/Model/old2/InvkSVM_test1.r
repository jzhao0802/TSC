...
##################################################################################################   
  ...
  if (kertype == "linear") {
  seev <-seq(1,  10, 100) # a set of values for C for binary SVMs
  ifelse(ONE,prmv <- nuv,prmv<- seev)
  ifelse(ONE,svmtype <- "one-classification",svmtype <- "C-classification")
  #ifelse(ONE,wtslst <- list(NULL),wtslst <- list(c("-1"=.05,"1"=1),  c("-1"=.1,"1"=1) ))
  ifelse(ONE,wtslst <- list(NULL),wtslst <- list(c("-1"=.05,"1"=1), c("-1"=.1,"1"=1) ))
...
#add plot end
   ...
   MaxIndex <- DRIndex[which( validResMat[TPIndex,3] == max(validResMat[TPIndex,3]))] 
	...
	#get result of training data with selprm and selwt[1]
	
	
	#ending of getting result of training data with selprm and selwt[1]
    ...
    get_perform_trainv <- function(){
			svm_model_tr <- svm(x = trainv$x, y = trainv$y, scale = FALSE, type = svmtype, nu = selprm,  cost = selprm, kernel = kertype, class.weights = selwt)
			pred <- predict(svm_model_tr,trainv$x,sclae=FALSE,decision.values = TRUE)
			 if (!ONE) {
				predscore <- attr(pred, "decision.values")
				...
				trainv_auc <- c(counter, ROCAUC, DR, TP, PPV, selprm, selwt[1]) #added by Jie
	...
	result_invkSVM <- list()
	result_invkSVM <- list(model=svm_model, score=predscoreVl, valid_auc=valid_auc, trainv_auc=trainv_auc)
